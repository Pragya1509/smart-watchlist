...
const route_errors_1 = require("./route-errors");
...
mainRouter.get('/', async (_req, res) => {
    try {
        ...
        (0, route_errors_1.sendRouteError)(res, error);
    }
});
/**
 * @openapi
...
mainRouter.get('/api/charts/equity-historical-data', async (req, res) => {
    try {
        const { symbol, start, end, token, symbolType = 'Equity', chartType = 'I', timeInterval = '5' } = req.query;
        // Validate required parameters
        if (!symbol) {
            return res.status(400).json({ error: 'Missing required parameter: symbol' });
        }
        // Call the charting method
        const parseChartDateParam = (value) => {
        ...
        const chartData = await nseIndia.getEquityChartHistoricalData(String(symbol), range, token ? String(token) : undefined, String(symbolType), String(chartType), String(timeInterval));
        res.json(chartData);
    }
    catch (error) {
        (0, route_errors_1.sendRouteError)(res, error);
...
mainRouter.post('/api/mcp/query', async (req, res) => {
    try {
        ...
        const originalDebugState = (0, mcp_client_js_1.getMcpClient)().isDebugLoggingEnabled();
        // Temporarily enable debug logging if requested
        if (shouldEnableDebug) {
            (0, mcp_client_js_1.getMcpClient)().setDebugLogging(true);
        }
        try {
            const result = await (0, mcp_client_js_1.getMcpClient)().processQuery({
                query,
                sessionId,
                userId,
                model,
            ...
            (0, mcp_client_js_1.getMcpClient)().setDebugLogging(originalDebugState);
        }
    }
    catch (error) {
...
mainRouter.get('/api/mcp/tools', async (_req, res) => {
    try {
        const tools = (0, mcp_client_js_1.getMcpClient)().getAvailableTools();
        res.json({ tools });
    }
    catch (error) {
...
mainRouter.get('/api/mcp/test', async (_req, res) => {
    try {
        ...
        const isConnected = await (0, mcp_client_js_1.getMcpClient)().testConnection();
        if (isConnected) {
            res.json({
                status: 'success',
...
mainRouter.get('/api/mcp/functions', async (_req, res) => {
    try {
        const functions = (0, mcp_client_js_1.getMcpClient)().getOpenAIFunctions();
        res.json({ functions });
    }
    catch (error) {
...
mainRouter.get('/api/mcp/session/:sessionId', async (req, res) => {
    try {
        ...
        const sessionInfo = (0, mcp_client_js_1.getMcpClient)().getSessionInfo(sessionId);
        if (!sessionInfo) {
...
mainRouter.get('/api/mcp/session/:sessionId/history', async (req, res) => {
    try {
        ...
        const history = (0, mcp_client_js_1.getMcpClient)().getConversationHistory(sessionId, maxMessages ? parseInt(maxMessages) : undefined);
        if (history.length === 0) {
            return res.status(404).json({
...
mainRouter.put('/api/mcp/session/:sessionId/preferences', async (req, res) => {
    try {
        ...
        (0, mcp_client_js_1.getMcpClient)().updateUserPreferences(sessionId, preferences);
        res.json({
...
mainRouter.delete('/api/mcp/session/:sessionId/clear', async (req, res) => {
    try {
        const { sessionId } = req.params;
        (0, mcp_client_js_1.getMcpClient)().clearSession(sessionId);
        res.json({
...
mainRouter.get('/api/mcp/session/:sessionId/export', async (req, res) => {
    try {
        const { sessionId } = req.params;
        const sessionData = (0, mcp_client_js_1.getMcpClient)().exportSessionData(sessionId);
        if (!sessionData) {
            return res.status(404).json({
                error: 'Session not found'
            });
...
mainRouter.post('/api/mcp/cleanup', async (_req, res) => {
    try {
        (0, mcp_client_js_1.getMcpClient)().cleanupExpiredSessions();
        res.json({
...
mainRouter.get('/api/mcp/session/:sessionId/context-stats', async (req, res) => {
    try {
        const { sessionId } = req.params;
        const stats = await (0, mcp_client_js_1.getMcpClient)().getContextStats(sessionId);
        res.json(stats);
    }
    catch (error) {
...
mainRouter.post('/api/mcp/session/:sessionId/summarize', async (req, res) => {
    try {
        const { sessionId } = req.params;
        const summary = await (0, mcp_client_js_1.getMcpClient)().forceContextSummarization(sessionId);
        if (!summary) {
            return res.status(404).json({
                error: 'Session not found'
            });
...
mainRouter.get('/api/mcp/session/:sessionId/context-window', async (req, res) => {
    try {
        const config = (0, mcp_client_js_1.getMcpClient)().getContextWindowConfig();
        res.json(config);
    }
    catch (error) {
...
mainRouter.put('/api/mcp/session/:sessionId/context-window', async (req, res) => {
    try {
        ...
        (0, mcp_client_js_1.getMcpClient)().updateContextWindowConfig(config);
        res.json({
            message: 'Context window configuration updated successfully',
            config: (0, mcp_client_js_1.getMcpClient)().getContextWindowConfig()
        });
    }
    catch (error) {
...
mainRouter.get('/api/mcp/session/:sessionId/summarization/last', async (req, res) => {
    try {
        const { sessionId } = req.params;
        if (!(0, mcp_client_js_1.getMcpClient)().isMemoryEnabled()) {
            return res.status(500).json({ error: 'Memory manager not enabled' });
        }
        const lastSummarization = (0, mcp_client_js_1.getMcpClient)().getLastSummarization(sessionId);
...
mainRouter.get('/api/mcp/session/:sessionId/summarization/history', async (req, res) => {
    try {
        ...
        const history = (0, mcp_client_js_1.getMcpClient)().getSummarizationHistory(sessionId, limit);
        res.json({
            sessionId,
...
mainRouter.get('/api/mcp/session/:sessionId/summarization/summary', async (req, res) => {
    try {
        ...
        const summary = (0, mcp_client_js_1.getMcpClient)().getSummarizationSummary(sessionId);
        if (!summary) {
            return res.status(404).json({
...
mainRouter.get('/api/mcp/session/:sessionId/openai-messages', async (req, res) => {
    try {
        ...
        const data = (0, mcp_client_js_1.getMcpClient)().getOpenAIMessages(sessionId);
        if (!data) {
