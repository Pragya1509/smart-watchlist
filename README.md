...
- **Real, user-configured price alerts** — set your own target price and direction ("notify me if TCS goes above ₹4200"); the backend checks this on every price update and raises a personal alert only you see.
- **Historical chart** — click any price to see a 60-day chart.
...
**How state persists across sessions/devices:** everything lives in MongoDB, keyed by user account — watchlist membership, last-viewed timestamps, and alert configuration are not stored client-side, so the experience is consistent across devices.

**How stale/delayed/conflicting data is handled:** the UI always shows the last known good snapshot, tagged with its age. If a live fetch fails, the error is classified (rate-limited vs. genuinely invalid symbol) and shown honestly rather than as a generic failure.

**How it scales:** the poller fetches each distinct symbol once per cycle regardless of how many users are watching it, decoupling upstream API load from user count.
...
- **Frontend:** React, Recharts (for historical charts)
